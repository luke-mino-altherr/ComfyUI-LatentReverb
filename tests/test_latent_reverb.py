...
class TestLatentReverbNode:
    ...
    def test_input_types(self):
        ...
        # INPUT_TYPES returns a tuple, so we need to access the first element
    ...
    def test_apply_reverb_dry(self, reverb_node, sample_latent_dict):
        ...
        result = reverb_node.apply_reverb(
            sample_latent_dict, wet_mix=0.0, feedback=0.0, room_size=0.5, num_reflections=8
        )
...
class TestErrorHandling:
    ...
    def test_invalid_wet_mix(self, reverb_node, sample_latent_dict):
        ...
        result = reverb_node.apply_reverb(
            sample_latent_dict, wet_mix=1.5, feedback=0.4, room_size=0.5, num_reflections=8
        )
    ...
    def test_empty_latent(self, reverb_node):
        ...
        with pytest.raises(Exception):
            reverb_node.apply_reverb(
                empty_latent, 0.3, 0.4, 0.5, 8
            )
...
class TestPerformance:
    ...
    def test_memory_usage(self, reverb_node, sample_latent_dict):
        ...
        for _ in range(5):
            result = reverb_node.apply_reverb(
                sample_latent_dict, 0.3, 0.4, 0.5, 8
            )
            del result
